...
class CWSClient {
	...
	public function __construct($token, $serviceKey, $merchProfileID = '', $workflowId = '', $applicationID = '', $session_token = '', $_svc = null) {
		...
		try {
			...
			/*$this->serviceInfo = new CWSServiceInformation("https://api.cert.vantiv.com/2.0.18/SvcInfo/".$serviceKey."?wsdl", array('trace'=>1, 'exceptions'=>1));
			 *$this->bankCard = new CWSBankcard("https://api.cert.vantiv.com/2.0.18/Txn/".$serviceKey."?wsdl", array('trace'=>1, 'exceptions'=>1));
			 */
